...
export async function POST(request: NextRequest) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_SYNC_REQUEST_BODY_BYTES) {
    return apiError(413, "payload_too_large", "Request payload too large")
  }
  ...
  try {
    ...
    if (error instanceof ZodError) {
      return apiError(400, "invalid_payload", "Invalid sync operations payload")
    }
    if (error instanceof Error && error.message === "Operation payload is too large") {
      return apiError(413, "payload_too_large", "Operation payload exceeds size limit")
    }
  ...
  if (error) {
    ...
    return apiError(500, "enqueue_failed", "Failed to enqueue")
  }
