...
import { API_V1_VERSION, type ApiV1InpatientSummaryResponse } from "@/lib/api/v1"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      .from("admissions")
      .select("status")
      .gte("admission_date", range.fromIso)
      .lte("admission_date", range.toIso)
      .limit(5000)

    if (error) {
      return apiError(500, "inpatient_summary_failed", "Failed to load inpatient summary", request)
    }
    ...
    const totals = ((rows || []) as Array<{ status?: string | null }>).reduce(
      ...
      {
        total_in_range: 0,
        admitted_or_active: 0,
        discharged: 0,
        other: 0,
      },
    )

    const payload: ApiV1InpatientSummaryResponse = {
      ok: true,
