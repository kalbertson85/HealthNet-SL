...
import { API_V1_VERSION, type ApiV1PatientsSummaryResponse } from "@/lib/api/v1"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [{ count: total }, { count: createdInRange }] = await Promise.all([
    ...
    const payload: ApiV1PatientsSummaryResponse = {
      ok: true,
      ...
      patients: {
        ...
        created_in_range: Number(createdInRange || 0),
      },
    ...
    return apiError(500, "patients_summary_failed", "Failed to load patients summary", request)
  }
}
