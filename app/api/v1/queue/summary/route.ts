...
import { API_V1_VERSION, type ApiV1QueueSummaryResponse } from "@/lib/api/v1"
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { supabase } = await requirePermission(request, "queue.manage")
    ...
    const { data: rows, error } = await supabase
      .from("queues")
      .select("status")
      .gte("check_in_time", range.fromIso)
      .lte("check_in_time", range.toIso)
      .limit(5000)

    if (error) {
      return apiError(500, "queue_summary_failed", "Failed to load queue summary", request)
    }
    ...
    const payload: ApiV1QueueSummaryResponse = {
      ok: true,
