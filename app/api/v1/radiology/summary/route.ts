...
import { API_V1_VERSION, type ApiV1RadiologySummaryResponse } from "@/lib/api/v1"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      return apiError(500, "radiology_summary_failed", "Failed to load radiology summary", request)
    }
    ...
    const payload: ApiV1RadiologySummaryResponse = {
      ok: true,
