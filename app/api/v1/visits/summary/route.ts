...
import { API_V1_VERSION, type ApiV1VisitsSummaryResponse } from "@/lib/api/v1"
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: visitsRows, error } = await supabase
      .from("visits")
      .select("visit_status")
    ...
    if (error) {
      return apiError(500, "visits_summary_failed", "Failed to load visits summary", request)
    }

    const visits = (visitsRows || []) as Array<{ visit_status?: string | null }>
    const totals = visits.reduce(
      (acc, row) => {
        ...
        if (status === "completed" || status === "discharged") {
          acc.completed_or_discharged += 1
        } else if (status.endsWith("_pending") || status === "doctor_review" || status === "admitted") {
          acc.pending += 1
          acc.active += 1
        } else {
          acc.active += 1
      ...
      },
      {
        ...
        active: 0,
        pending: 0,
        completed_or_discharged: 0,
      },
    )

    const payload: ApiV1VisitsSummaryResponse = {
      ok: true,
