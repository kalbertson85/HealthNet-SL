...
describe("SQL SECURITY DEFINER search_path coverage", () => {
  it("ensures SECURITY DEFINER functions specify explicit SET search_path", async () => {
    ...
    for (const file of files) {
      ...
      while (true) {
        ...
        const isSecurityDefiner = block.includes("security definer")
        if (isSecurityDefiner && !block.includes("set search_path")) {
          const headerEnd = block.indexOf("as $$")
          const header = block.slice(0, headerEnd === -1 ? 240 : Math.min(headerEnd, 240)).replace(/\s+/g, " ").trim()
          violations.push(`${file.name}: SECURITY DEFINER function missing SET search_path (${header.slice(0, 180)})`)
        }
